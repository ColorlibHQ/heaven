 ...
 */
	...
	final class Heaven{
		...
		private function setup(){
			...
			$enqueu = new heaven_Enqueue();
			$enqueu->scripts = $this->enqueue() ;
			$enqueu->heaven_scripts_enqueue_init() ;

		}
		// Theme Support
		public function support(){
			// content width
	        $GLOBALS['content_width'] = apply_filters( 'heaven_content_width', 751 );

	        
	        // text domain for translation.
	        load_theme_textdomain( 'heaven', HEAVEN_DIR_PATH . '/languages' );
			...
			// Site logo size
			add_image_size( 'heaven_logo_127x21', 127, 21, true );
					
			// About section image size
			add_image_size( 'heaven_about_img_1110x741', 1110, 741, true );
			add_image_size( 'heaven_about_service_img_750x447', 750, 447, true );
						
			// Service image size
			add_image_size( 'heaven_service_image_540x628', 540, 628, true );
						
			// Portfolio image size
			add_image_size( 'heaven_portfolio_image_540x516', 540, 516, true );
			add_image_size( 'heaven_portfolio_single_image_1140x583', 1140, 583, true );
			add_image_size( 'heaven_portfolio_gallery_img_555x502', 555, 502, true );

			// Home blog post image size
			add_image_size( 'heaven_latest_blog_712x421', 712, 421, true );

			// Latest post & Portfolio author thumbnail Widget size
			add_image_size( 'heaven_widget_post_thumb', 80, 80, true );

			// Single blog post image size
			add_image_size( 'heaven_single_blog_750x375', 750, 375, true );
			add_image_size( 'heaven_np_thumb', 60, 60, true );
	        ...
	        register_nav_menus( array(
	            'primary-menu'   => esc_html__( 'Primary Menu', 'heaven' ),
	            'projects-menu'  => esc_html__( 'Projects', 'heaven' )
			) );
		...
		private function enqueue(){
			...
			$cssPath = HEAVEN_DIR_CSS_URI;
			$jsPath  = HEAVEN_DIR_JS_URI;

			$scripts = array(
				'style' => array(
					array(
						'handler'		=> 'heaven-wp-theme-google-font',
					...
					array(
						'handler'		=> 'heaven-wp-theme-animate',
						'file' 			=> $cssPath.'animate.css',
						'dependency' 	=> array(),
						'version' 		=> '1.0',
					),
					array(
						'handler'		=> 'heaven-wp-theme-owl-carousel',
						'file' 			=> $cssPath.'owl.carousel.min.css',
						'dependency' 	=> array(),
						'version' 		=> '2.3.4',
					),
					array(
						'handler'		=> 'heaven-wp-theme-font-awesome',
						'file' 			=> $cssPath.'font-awesome.min.css',
						'dependency' 	=> array(),
						'version' 		=> '7.3.1-1',
					),
					array(
						'handler'		=> 'heaven-wp-theme-themify',
					...
					array(
						'handler'		=> 'heaven-wp-theme-slick-css',
					...
					array(
						'handler'		=> 'heaven-wp-theme-heaven-style',
		...
		private function google_font(){
			 ...
			 */
			if ( 'off' !== _x( 'on', 'Google font: on or off', 'heaven' ) ) {
		...
		private function customizer_init(){
			...
			// Instantiate heaven theme customizer
			$heaven_theme_customizer = new heaven_theme_customizer();
		}
	} // End Heaven Class
