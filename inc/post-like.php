 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_heaven_process_simple_like', 'heaven_process_simple_like' );
add_action( 'wp_ajax_heaven_process_simple_like', 'heaven_process_simple_like' );
function heaven_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'heaven' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'heaven' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'heaven' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'heaven' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'heaven' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'heaven' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'heaven_show_user_likes' );
add_action( 'edit_user_profile', 'heaven_show_user_likes' );
function heaven_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'heaven' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'heaven' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
